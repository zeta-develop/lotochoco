...
export async function createResult(data: {
  ...
  const results = await query<Result>('SELECT * FROM "Result" WHERE id = ?', [
    id,
  ]);
  const result = results[0];

  // Incluir relaciones
  const games = await query<Game>('SELECT * FROM "Game" WHERE id = ?', [
    result.gameId,
  ]);
  result.game = games[0];

  const schedules = await query<DrawSchedule>(
    'SELECT * FROM "DrawSchedule" WHERE id = ?',
    [result.scheduleId],
  );
...
export async function getTodayResults(): Promise<Result[]> {
  try {
    ...
    if (results.length > 0) {
      const gameIds = [...new Set(results.map((r) => r.gameId))].filter(
        Boolean,
      );
      const scheduleIds = [...new Set(results.map((r) => r.scheduleId))].filter(
        Boolean,
      );
      ...
      const schedulesMap = new Map<string, DrawSchedule>();
      if (scheduleIds.length > 0) {
        const placeholders = scheduleIds.map(() => "?").join(", ");
        const schedules = await query<DrawSchedule>(
          `SELECT * FROM "DrawSchedule" WHERE "id" IN (${placeholders})`,
          scheduleIds,
        );
      ...
      for (const r of results) {
        ...
        if (r.gameId) r.game = gamesMap.get(r.gameId);
        if (r.scheduleId) r.schedule = schedulesMap.get(r.scheduleId);
      }
    }
...
export async function getWinners(options?: {
  ...
  for (const w of winners) {
    ...
    const tickets = await query<any>("SELECT * FROM Ticket WHERE id = ?", [
      w.ticketId,
    ]);
    w.ticket = tickets[0];
    const results = await query<any>("SELECT * FROM Result WHERE id = ?", [
      w.resultId,
    ]);
    w.result = results[0];
  }
...
export async function markWinnerAsPaid(winnerId: string): Promise<void> {
  await execute(
    "UPDATE Winner SET isPaid = 1, paidAt = CURRENT_TIMESTAMP WHERE id = ?",
    [winnerId],
  );

  // Actualizar sesión de caja (simplificado para este ejemplo)
  const session = await query(
    'SELECT id FROM CashSession WHERE status = "open" LIMIT 1',
  );
  if (session.length > 0) {
    const winner = (
      await query<Winner>("SELECT prizeAmount FROM Winner WHERE id = ?", [
        winnerId,
      ])
    )[0];
    await execute(
      "UPDATE CashSession SET prizesTotal = prizesTotal + ? WHERE id = ?",
