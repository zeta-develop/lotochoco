...
## 2024-05-24 - Acoplamiento ineficiente de Hooks
**Aprendizaje:** En `hooks/use-results.ts`, los hooks secundarios `useWinners` y `usePendingWinners` llamaban internamente a `useResults()` únicamente para extraer y reexportar su estado `isLoading` y la función `refresh`. Dado que `useResults()` siempre ejecuta una consulta asíncrona por defecto al inicializarse (`getTodayResults`), instanciar estos hooks en un mismo componente (como en `Dashboard`, que llama tanto a `useTodayResults` como a `usePendingWinners`) provoca que `getTodayResults` se dispare múltiples veces de forma redundante y paralela.
**Acción:** Desacoplar la ejecución. Evitar componer hooks complejos solo para reutilizar banderas booleanas. Se ha refactorizado para que cada hook gestione su propio estado asíncrono y funciones de recarga independientes, reduciendo significativamente la carga inicial del `Dashboard` y evitando consultas n+1 en el cliente.
